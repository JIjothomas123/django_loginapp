...
from loginapp.models import registration,admin,studentregistration,attendances,marksubmit,fac_leave_management,stu_leave_management,attendance_submit
...
def Addattendance(request):
    ...
    return render(request,'Addattendance.html',{'authors':queryset}) 
...
def stuleave(request):
    ...
    return render(request,'stuleave.html',{'authors':queryset}) 
...
def stuleaveapprove(request):
    queryset = stu_leave_management.objects.all()
    return render(request,'stuleaveapproval.html',{'authors':queryset})  
def facapproval(request):
    
    if request.method=='POST': 
        fname=request.POST.get('fname')
        
        fac_leave_management.objects.filter(name=fname).update(status='Approved')
        return render(request,'facleaveapprove.html')
def stuapproval(request):
     
    if request.method=='POST': 
        fname=request.POST.get('fname')
        
        stu_leave_management.objects.filter(name=fname).update(status='Approved')
        return render(request,'stuleaveapproval.html')        

def studentmark(request):
    query = marksubmit.objects.all().filter(student_name=request.session['usr1'])
    return render(request,'marklist.html',{'authors':query})
  
def viewattendance(request):
    y = attendances.objects.all().filter(student_name=request.session['usr1'])
    return render(request,'viewattendance.html',{'authors':y})
def logout_view(request):
    logout(request)
    return render(request,'index.html') 
def facultydet(request):
    queryset =registration.objects.all()
    return render(request,'facultydet.html',{'authors':queryset})
def studentdet(request):
    queryset =studentregistration.objects.all()
    return render(request,'studentdet.html',{'authors':queryset}) 
def attsub(request):
    status=request.POST.get('status')
    ...
    x=attendances.objects.all().filter(student_name=name)[0]
    if name==x.student_name and date==x.Date and hour=="1":
        x.status_h1=status
        x.save(['status_h1'])
    elif name==x.student_name and date==x.Date and hour=="2":
        attendances.objects.filter(student_name=name).update(status_h2=status)
    elif name==x.student_name and date==x.Date and hour=="3":
        attendances.objects.filter(student_name=name).update(status_h3=status)
    elif name==x.student_name and date==x.Date and hour=="4":
        attendances.objects.filter(student_name=name).update(status_h4=status)
    elif name==x.student_name and date==x.Date and hour=="5":
        attendances.objects.filter(student_name=name).update(status_h5=status)
    else:
        a=attendances(student_name=name,Date=date)
        a.save()
